...
RSpec.describe CircleCI::Parallel do
  ...
  before do
    @invoked_before_join_hook = false
    @invoked_after_join_hook = false
    @merged_data = {}
  ...
  context 'on CIRCLE_NODE_INDEX 0 node', if: ENV['CIRCLE_NODE_INDEX'] == '0' do
    ...
    it 'invokes all hooks' do
      expect { CircleCI::Parallel.join }
        .to change { @invoked_before_join_hook }.from(false).to(true)
        .and change { @invoked_after_join_hook }.from(false).to(true)
        .and change { @merged_data }.to(expected_merged_data)
    end
  end

  context 'on CIRCLE_NODE_INDEX 1 node', if: ENV['CIRCLE_NODE_INDEX'] == '1' do
    it 'invokes only before_join and after_join hooks' do
      expect { CircleCI::Parallel.join }
        .to change { @invoked_before_join_hook }.from(false).to(true)
        .and change { @invoked_after_join_hook }.from(false).to(true)
